...
from Measures import get_snr,get_rmse,get_quality
            ...
            for id_model in Models:
                ...
                for Qmax in range(Qmax_test, Qmax_max, 1):
                    ...
                    for Qmin in range(Qmax_test,0,- 1):#:#:(1,Qmax_test,1):#
                        ...
                        quality_test = get_quality(coefs_DWT, coefs_DWT_q_dec,metric)
                        
                
                            
                        #print(Models[id_model]["family"],Qmin, Qmax, "R={:.2f} b/s".format((R_real + R_h[id_signal][id_phase][w]) / N), end_Na, "{}={:.2f}".format(metric,quality_test))
                        # Check if the current bit rate does not exceed the total allowed budget
                        
                        ntot_test=R_real + R_h[id_signal][id_phase][w]
                        if quality_test<quality:#R_real + R_h[id_signal][id_phase][w] <= n_tot:   
                            # Check if the current RMSE is better than the previous model's RMSE
                            if ntot_test < ntot_model:
                                
                                # Update the best RMSE found so far for the current model
                                ntot_model = ntot_test
                                
                                # If this is the best RMSE found overall, update the best parameters
                                if ntot_test < ntot_min:
                                    ntot_min = ntot_test
                                    
                                    # Store the best Signal-to-Noise Ratio (SNR) and RMSE
                                    SNR[id_signal][id_phase][w] = SNR_test
                                    RMSE[id_signal][id_phase][w] = get_rmse(coefs_DWT, coefs_DWT_q_dec)
                            ...
                            # If quality_test is less than quality, we stop testing higher Qmax values and decrease Qmin to reduce the number of bits.
                            end_Na = 1                            
